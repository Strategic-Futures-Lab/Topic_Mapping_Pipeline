 ...
 *
 * Version tweaked by Tom for speed reasons, allowing disabling of serialisation etc.
 */
...
public class TopicModel implements Serializable
{
    ...
    //MOVED STATIC FINAL SO YOU CAN CALCULATE MULTIPLE TOPIC MODELS!
    //public static final int SEED = 1;
    //public static final int ITER = 500;
    ...
    public TopicModel(List<TopicRowContainer> map)
    {
        this.map = map;
    }
    ...
    public void Model(boolean serialiseModel, boolean recreateCorpus, String rotLoc)
    {
        ...
        if(recreateCorpus)
        {
            ...
            try
            {
                FileWriter writer = new FileWriter(rotLoc + File.separator + "corpus.txt");

                int count = 0;
                for (TopicRowContainer entry : map)
                {
                    if(entry != null)
                    {
        ...
        try{
            fileReader = new InputStreamReader(new FileInputStream(new File(rotLoc + File.separator + "corpus.txt")), StandardCharsets.UTF_8);
        }
        catch(Exception e){
        ...
        //  Saving topics
        topics = new ArrayList<Topics>(model.numTopics);
        ...
        for (int topic = 0; topic < model.numTopics; topic++) {
            ...
            Topics topicTemp = new Topics(topic, topicSortedWords.get(topic).size(), labels, labelsID, weights);
    ...
    public double[] InferTopics(String UID, String text, StanfordLemmatizer slem, boolean serverProcess){
        ...
        try{
            ...
            Writer writerL = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(lemmatizedFileName), StandardCharsets.UTF_8));
            ...
            if(serverProcess){
                ...
                try {                           // Save file if error
                    Writer writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(errorFile), StandardCharsets.UTF_8));
