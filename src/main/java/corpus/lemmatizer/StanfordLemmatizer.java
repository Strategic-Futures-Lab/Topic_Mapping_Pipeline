...
public class StanfordLemmatizer {
     ...
     */
    public List<List<String>> lemmatise(String documentText) {
        ...
        for(CoreMap sentence: sentences) {
            List<String> sentenceLemmas = new LinkedList<>();
            // Iterate over all tokens in a sentence
            for (CoreLabel token: sentence.get(CoreAnnotations.TokensAnnotation.class)) {
                // Retrieve and add the lemma for each word into the
                // list of lemmas
                String lemma = token.get(CoreAnnotations.LemmaAnnotation.class);
                if(!lemma.equals(".")) sentenceLemmas.add(lemma);
            }
            lemmas.add(sentenceLemmas);
        }
        return lemmas;
    }
     ...
     * @param sentences List of lemmas to clean
     * @return The cleaned list of lemmas
     */
    public static List<List<String>> removeCommonStopWords(List<List<String>> sentences){
        ...
        for(List<String> sentence: sentences) {
            ...
            for (String word : sentence) {
                ...
                // Check if word is only non-alphanumeric
                if (!foundStopWord) {
                    foundStopWord = word.matches("\\W+");
                }
                if (!foundStopWord) {
                    foundStopWord = word.matches("[^a-zA-Z]+");
                }
            ...
            if(!newWords.isEmpty()) newSentences.add(newWords);
        }
        return newSentences;
    }
}
