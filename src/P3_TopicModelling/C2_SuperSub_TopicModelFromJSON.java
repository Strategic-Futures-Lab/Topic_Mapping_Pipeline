...
/**
 * Created by Azimeh on 31/07/2019.
 */
public class C2_SuperSub_TopicModelFromJSON {
    ...
    private int numSuperTopics, numSubTopics, numIterations, numWords, numDocuments;

    private String inputFilename, outputFilenameSuper, outputFilenameSub;
     ...
     *
     *
     * @param args - [JSON Location] [Output Location Super] [Output Location Sub] [Number of Super Topics] [Number of Sub Topics] [Number of Iterations] [Number of Words] [Number of Documents]
     */
    public static void main(String[] args) {
        System.out.println("\n********************************************\n" +
                "*                                          *\n" +
                "* STARTING C2_SuperSub_TopicModelFromJSON!    *\n" +
                "*                                          *\n" +
                "* C2_SuperSub_TopicModelFromJSON :START    *\n" +
                "*                                          * \n" +
                "********************************************\n");

        C2_SuperSub_TopicModelFromJSON startClass = new C2_SuperSub_TopicModelFromJSON();
        startClass.StartTopicModelling(args);

        System.out.println("\n********************************************\n" +
                "*                                          *\n" +
                "* C2_SuperSub_TopicModelFromJSON PHASE COMPLETE!    *\n" +
                "*                                          *\n" +
                "* C2_SuperSub_TopicModelFromJSON:END                *\n" +
                "*                                          * \n" +
                "********************************************\n");
    }
    ...
    private void StartTopicModelling(String[] args) {
        ...
        calculateTopicToTopicCosineDistances(outputFilenameSub, df);
        SaveJSON(outputFilenameSub);
    }
    ...
    private void checkArgs(String[] args) {
        if (args.length < 3) {
            ...
            System.out.println("\nTopic Model arguments missing. Continuing with defaults -> 30 topics, 2000 iterations, 50 words, 100 Documents.\n\nPlease supply arguments in the following order: " +
                    "[JSON Location] [Output Location Super] [Output Location Sub] [Number of Super Topics] [Number of Sub Topics] [Number of Iterations] [Number of Words] [Number of Documents]");
            inputFilename = args[0];
            outputFilenameSuper = args[1];
            outputFilenameSub = args[2];
            numSuperTopics = 60;
            numSubTopics = 300;
            numIterations = 2000;
            numWords = 50;
            ...
            outputFilenameSub = args[2];
            try {
                numSuperTopics = Integer.parseInt(args[3]);
                numSubTopics = Integer.parseInt(args[4]);
                numIterations = Integer.parseInt(args[5]);
                numWords = Integer.parseInt(args[6]);
                numDocuments = Integer.parseInt(args[7]);
    ...
    private void runTopicModels() {
        ...
        System.out.println("\n***********\n Running Super Topic Model! \n************\n");
        runSingleTopicModel(0);

        System.out.println("\n***********\n Running Sub Topic Model! \n************\n");
        runSingleTopicModel(1);
    ...
    private void runSingleTopicModel(int number) {
        ...
        if(number == 0 )
            tModel.numTopics = numSuperTopics;
        else
            tModel.numTopics = numSubTopics;


        TopicModel.name = "topicModelNum" + number;
    ...
    private void calculateTopicToTopicCosineDistances(String outputFilename, DecimalFormat df) {
        ...
        double[][] simMatrix = new double[numSuperTopics][numSuperTopics];

        double[][] topicVector = getTransposedMatrix(tModel.topicDistributions, 0.1f);

        int x, y;

        for (int yTopic = 0; yTopic < numSuperTopics; yTopic++) {
            for (int xTopic = 0; xTopic < numSuperTopics; xTopic++) {

                try{
                    simMatrix[xTopic][yTopic] = Double.valueOf(df.format(cosineSimilarity(topicVector[xTopic], topicVector[yTopic])));
                    System.out.println(topicVector[xTopic]);
    ...
    private void saveCosineDistanceCSV(String outputFilename, double[][] simMatrix) {
        ...
        try (CsvAppender csvAppender = csvWriter.append(file, StandardCharsets.UTF_8)) {
            String[] labels = new String[numSuperTopics];
            csvAppender.appendField("");
            //Create the labels...
            for (int topic = 0; topic < numSuperTopics; topic++) {
                labels[topic] = "";
    ...
    private void SaveJSON(String JSONFile) {
        ...
        metadata.put("numTopics", "" + numSuperTopics);
        ...
        jWrapper.SetRowDataLimited(JSONRows);
