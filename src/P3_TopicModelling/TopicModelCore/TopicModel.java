...
public class TopicModel implements Serializable {
    ...
    /** used to record log-likelihood after modelling process */
    public transient LogLikelihoodRecord logLikelihoodRecord;
    /** used to record history of topics */
    public transient TopicRecord topicRecord;

    /** Create empty instance for inference, attributes loaded from serialised model */
    public TopicModel(){}
    /** Create instance with list of documents */
    ...
    public void Model(String outputDir){
        ...
        // Write the documents in MALLET format
        try {
            File file = new File(outputDir+File.separator+"modelCorpus.txt");
        ...
        try{
            fileReader = new InputStreamReader(new FileInputStream(new File(outputDir + File.separator + "modelCorpus.txt")), "UTF-8");
        } catch(Exception e){
        ...
        //   second the sumber of words to print per topics
        ...
        logLikelihoodRecord = new LogLikelihoodRecord(logHandler.getLLRecords(), model.totalTokens, model.modelLogLikelihood(), ITER);
        // recording history of topics
        topicRecord = new TopicRecord(logHandler.getTopicRecords(), TOPICS, ITER, topicPrintInterval);
    ...
    /** Infer a lemmatised text, returning the topic distribution for that text */
