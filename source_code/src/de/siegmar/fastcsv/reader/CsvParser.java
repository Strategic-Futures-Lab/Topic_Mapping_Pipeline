...
public final class CsvParser implements Closeable {
    ...
    private void initHeader(final List<String> currentFields) {
        ...
        for (int i = 0; i < currentFields.size(); i++) {
            ...
            if (field != null && !field.isEmpty() && !localHeaderMap.containsKey(field)) {
                //CODE MODIFIED BY TOM TO STRIP QUOTES FROM HEADER COLUMNS
                localHeaderMap.put(stripLeadingAndTrailingQuotes(field), i);
            }
        }
    ...
    /**
     * ADDITIONAL CODE ADDED BY TOM TO STRIP QUOTES FROM HEADER COLUMNS!
     */
